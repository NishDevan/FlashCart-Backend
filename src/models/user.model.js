...
class User {
    ...
    static async findByEmail(email) {
        const query = 'SELECT id, username, email, password, created_at FROM users WHERE email = $1';
        const values = [email];
        const result = await db.query(query, values);
        return result.rows[0];
    }
    ...
    static async update(id, { username, email, password }) {
        ...
        const updateQuery = 'UPDATE users SET username = $1, email = $2, password = $3 WHERE id = $4 RETURNING id, username, email, created_at';
